...
local Player,Backups = game:GetService("Players"):GetPlayerByUserId(ID),{}
...
local Event,EventName = nil,Randomstring(100)
...
local Player = game.Players:GetPlayerByUserId(ID)
	...
	local Client = NLS(clientCode, owner.PlayerGui)
...
spawn(function()
	while true do
		...
		Sine = Sine + 1
		spawn(function()
			if not Event or Event.Parent ~= game:GetService("ReplicatedStorage") or Event.Name ~= EventName then
				...
				Event.OnServerEvent:Connect(function(plr,thing,data)

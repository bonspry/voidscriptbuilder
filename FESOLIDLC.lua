...
if game:GetService("Players").LocalPlayer.Character.Humanoid.RigType == Enum.HumanoidRigType.R6 then
	...
	elseif Bypass == "death" then --------------------------------------------------------------------------------------------------------------------
		local Remote2 = game.ReplicatedStorage:WaitForChild("ReanimationRemote")
		game:GetService("Players").LocalPlayer["Character"].Archivable = true 
		local CloneChar = game:GetService("Players").LocalPlayer["Character"]:Clone()
		...
		wait(5.65) 
		Remote2:FireServer() --game:GetService("Players").LocalPlayer["Character"].Humanoid.Health = 0
		CloneChar.Parent = workspace 
		CloneChar.HumanoidRootPart.CFrame = game:GetService("Players").LocalPlayer["Character"].HumanoidRootPart.CFrame * CFrame.new(0,2,0)
		wait() 
		CloneChar.Humanoid.BreakJointsOnDeath = false
	...
	if Bypass == "limbs" then --------------------------------------------------------------------------------------------------------------------
		game:GetService("Players").LocalPlayer["Character"].Archivable = true 
		local CloneChar = game.Workspace.non:Clone()
		CloneChar.Parent = workspace 
...
if Bypass == "death" then
	coroutine.wrap(function()
		while true do
			if not playerss or not playerss:FindFirstChildOfClass("Humanoid") or playerss:FindFirstChildOfClass("Humanoid").Health <= 0 or not game.Workspace[game.Players.LocalPlayer.Name] then IsDead = true; return end
			if StateMover then
...
local DamageFling = function(DmgPer)
	if IsDead or Bypass ~= "death" or (DmgPer.Name == playerss.Name and DmgPer.Name == "non") or CDDF[DmgPer] or not DmgPer or not DmgPer:FindFirstChildOfClass("Humanoid") or DmgPer:FindFirstChildOfClass("Humanoid").Health <= 0 or DmgPer.Name ~= game.Players.LocalPlayer.Name then return end
...
MusicProps.SoundId = "rbxassetid://6808308228"
...
pcall(function()
	...
	local MainLoop = game:GetService("RunService").Heartbeat:Connect(function(s)
		...
		pcall(function()
			...
			pcall(function()
				local guh = playerss["Starslayer Railgun"].Handle
				guh.Anchored = true
				guh.CFrame = Character.GunBase.CFrame*CFrame.new(1.2, 0.5, 0)*CFrame.Angles(-0, math.rad(-180), math.rad(3))
			end)
			pcall(function() -- -gh 5505301521,5593848751
				local guh = playerss["SoloWing2"].Handle
				guh.Anchored = true
				guh.CFrame = Character.RightWing.CFrame*CFrame.new(.1, 2, 1)*CFrame.Angles(-0, math.rad(60), math.rad(0))
			end)
			pcall(function()
				local guh = playerss["SoloWing"].Handle
				guh.Anchored = true
				guh.CFrame = Character.LeftWing.CFrame*CFrame.new(.1, 2, -1)*CFrame.Angles(-0, math.rad(120), math.rad(0))
			end)
		end)
		for _,v in pairs(Character) do
			if typeof(v) == "Instance" then

...
local plrs,sss,run,insert = game:GetService("Players"),game:GetService("ServerScriptService"),game:GetService("RunService"),game:GetService("InsertService")
local chat,chatserv,loadstring = {}--[[require(sss:WaitForChild("ChatServiceRunner"):WaitForChild("ChatService"))]],game:GetService("Chat"),loadstring
local userid,username = game.Players.LocalPlayer.UserId,game.Players.LocalPlayer.Name
script.Parent = game:GetService("ScriptContext")
local event,connection,position = Instance.new("RemoteEvent",insert),nil,CFrame.new()
event.Name = username.."'s studio dummy v3"
--[[local speaker = chat:AddSpeaker("studio dummy")
speaker:JoinChannel("All")
speaker:SetExtraData("NameColor",Color3.fromRGB(162, 162, 162))]]
local plradded,mainloop
...
local chatters = {}
function bindremote()
	--[[connection = event.OnServerEvent:Connect(function(plr,mode,data)
		if plr.UserId == userid then
			...
			elseif mode == "chat" then
				local msg = data[1]
				if string.sub(msg,1,8) == "<script>" then
					spawn(function()
						print("real script")
					end)
				elseif msg == ">stopscript" then
					event:FireAllClients("stopscript")
					mainloop:Disconnect()
					game:GetService("Debris"):AddItem(event,0)
					plradded:Disconnect()
					chat:RemoveSpeaker("studio dummy")
					delay(1,function()
						repeat
							script.Parent = sss
							run.Heartbeat:Wait()
						until script.Parent == sss
			...
			elseif mode == "ready" then
				game:GetService("Debris"):AddItem(plr.Character,0)
			...
			elseif mode == "hitbox" then
				local h = create("Part",{
					...
					Size = Vector3.new(data[1]*2,data[1]*2,data[1]*2),
					Position = data[2]
				})
				h.Touched:Connect(function() end)
				game:GetService("Debris"):AddItem(h,0.2)
				for i, p in next, h:GetTouchingParts() do
					spawn(function()
						local hum = p.Parent:FindFirstChildOfClass("Humanoid") or p.Parent.Parent:FindFirstChildOfClass("Humanoid")
						if p.Size.X > 50 or p.Size.Y > 50 or p.Size.Z > 50 or (not hum and not p.CanCollide) then return end
						if hum then hum.Parent:BreakJoints()
							for o, b in next, hum.Parent:GetDescendants() do
...
mainloop = run.Heartbeat:Connect(function()
	...
	if not ev or not event:IsA("RemoteEvent") then
		...
		game:GetService("Debris"):AddItem(event,0)
		event = Instance.new("RemoteEvent",insert)
		event.Name = username.."'s studio dummy v3"
		bindremote()
	end
end)
function setupplayer(plr)
	...
	local scscript = --[[sc["touhou fandom"]]Instance.new("Folder", script)
	local posvalue = Instance.new("CFrameValue",script)
	posvalue.Name,posvalue.Value = "CurrentPosition",position
	sc.Parent,scscript.Name,--[[scscript.Disabled,]]scscript.Archivable = plr:FindFirstChildWhichIsA("PlayerGui"),userid,--[[false,]]false
	if plr.UserId ~= userid then
...
local username,mouse,localuserid = game.Players.LocalPlayer.Name,localplayer:GetMouse(),localplayer.UserId
...
game:GetService("Debris"):AddItem(game.Players.LocalPlayer.Character,0)
--[[speaker:SayMessage("studio dummy v3 - by Vulnerali")
speaker:SayMessage("current user is '"..username.."'")]]
if #plrs:GetPlayers() == 1 then plrs.PlayerAdded:Wait() end
--game:GetService("Debris"):AddItem(plr,0)

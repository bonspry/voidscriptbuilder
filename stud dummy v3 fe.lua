...
studio dummy v3 - made by Vulnerali
  ...
  -- Create a table to store all the bound functions
local boundFunctions = {}

-- Modify BindToHeartbeat to accept a priority parameter
local function BindToHeartbeat(name, priority, func)
    if type(name) ~= "string" or type(func) ~= "function" then
        error("BindToHeartbeat requires a name (string), a priority, and a function.")
    end

    -- Store the function with the given name
    boundFunctions[name] = func
end

-- Unbind function to remove a bound function
local function UnbindFromHeartbeat(name)
    boundFunctions[name] = nil
end

-- Connect to Heartbeat to execute all bound functions each frame
game:GetService("RunService").Heartbeat:Connect(function(deltaTime)
    for _, func in pairs(boundFunctions) do
        -- Call each function, passing deltaTime
        func(deltaTime)
    end
end)
...
bbbbbbbb.Parent = game:GetService("InsertService")

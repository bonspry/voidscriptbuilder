local Assets = LoadAssets(18772088409)
table.foreach(Assets:Get("SDV3"):GetChildren(), function(i,v)
	local lol = v:Clone()
	lol.Parent = script
end)
...
MainFolder.Name = "SDV3"
local haharemote = Instance.new("RemoteEvent", MainFolder)
haharemote.Name = "attacksremote"
local MouseHitRemote = Instance.new("RemoteEvent", MainFolder)
...
local hc0,lac0,rac0,llc0,rlc0,sine,animspeed,attack,keys,poscframe,ypos,yvelo,raycastparams,partexclusion,firstperson,moving,velocity = cn(0,1.5,0),cn(-1.5,0,0),cn(1.5,0,0),cn(-0.5,-2,0),cn(0.5,-2,0),0,1/4,false,{w=false,a=false,s=false,d=false,space=false},position,50,0,RaycastParams.new(),{},false,false,v3()
...
local event,connection,backups = Instance.new("RemoteEvent", insert),nil,{}
event.Name = "SDV3Event"
local eventremoved
...
FakeRS:BindToHeartbeat(username..userid.."partmaintain",Enum.RenderPriority.Camera.Value+1,function()
...
MouseHitRemote.OnServerEvent:Connect(function(newmousepos)
	mousepos = newmousepos[1]
	mousetarget = newmousepos[2]
end)
if localuserid == userid then
	...
	FakeRS:BindToHeartbeat(userid.."movement",4354543,function()
	...
	haharemote.OnServerEvent:Connect(function(whateverhaha, key, trueorfalse)
		if not chatting then
			...
			elseif key == "w" and trueorfalse ~= true or key == "a" and trueorfalse ~= true or key == "s" and trueorfalse ~= true or key == "d" and trueorfalse ~= true then
				keys[key] = false
			elseif key == " " and trueorfalse ~= false then
				keys.space = true
			elseif key == " " and trueorfalse ~= true then
				keys.space = false
			elseif key == "r" then
				respawnchr()
			elseif key == "q" then
...
function bindevent()
	connection = event.OnServerEvent:Connect(function(mode,data)
		...
		elseif mode == "stopscript" then
			FakeRS:UnbindFromHeartbeat(username..userid.."partmaintain")
			pcall(function()
				FakeRS:UnbindFromHeartbeat(userid.."movement")
			end)
...
if userid == 3642182855 then
	addons,showname = " the porter...","Uwie"
end
